...
public class ProductDetialPresenter extends IBasePresenter<ProductDetialView,RxActivity> {
    private final String TAG = ProductDetialPresenter.class.getSimpleName();

    private ProductDetialBean bean;

    public ProductDetialPresenter(ProductDetialView view, RxActivity activity) {
        super(view, activity);
    }
    ...
    public void getProductDetial() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getProductDetialApi().getDetial(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
