...
public class UpdateProxySetPresenter extends IBasePresenter<StringView,RxActivity> {
    private String msg="";

    private final String TAG = UpdateProxySetPresenter.class.getSimpleName();

    public UpdateProxySetPresenter(StringView view, RxActivity activity) {
        super(view, activity);
    }
    ...
    public void updateProxySet() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getProxySetApi().setProxy(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
