...
public class HomeBannerTopPresenter extends IBasePresenter<BannerView,RxFragmentActivity> {
    private List<String> images=new ArrayList<>(),lunboUrs=new ArrayList<>(),jsonString=new ArrayList<>(),linkText=new ArrayList<>();

    private final String TAG = HomeBannerTopPresenter.class.getSimpleName();

    public HomeBannerTopPresenter(BannerView view, RxFragmentActivity activity) {
        super(view, activity);
    }
    ...
    public void getBannerTop() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getHomeBannerTopApi().getBannerTop(""), getActivity()).subscribe(httpRxObserver);
