...
public class LoginCodePresenter extends IBasePresenter<StringView,RxActivity> {
    private String msg="";

    private final String TAG = LoginCodePresenter.class.getSimpleName();

    public LoginCodePresenter(StringView view, RxActivity activity) {
        super(view, activity);
    }
    ...
    public void getCode() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getLoginCodeApi().getCode(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
