...
public class UpdateAvtarPresenter extends IBasePresenter<StringView,RxActivity> {
    private String msg="";

    private final String TAG = UpdateAvtarPresenter.class.getSimpleName();

    public UpdateAvtarPresenter(StringView view, RxActivity activity) {
        super(view, activity);
    }
     ...
     * 修改头像
     */
    public void updateAvtar(List<MultipartBody.Part> partList) {
        ...
        HttpRxObservable.getObservable(ApiUtils.getUpdateAvtarApi().updateAvtar(partList), getActivity()).subscribe(httpRxObserver);
    }
}
