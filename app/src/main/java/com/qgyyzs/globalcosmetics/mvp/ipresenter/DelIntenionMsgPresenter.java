...
public class DelIntenionMsgPresenter extends IBasePresenter<StringView,RxFragmentActivity> {
    private String msg="";

    private final String TAG = DelIntenionMsgPresenter.class.getSimpleName();

    public DelIntenionMsgPresenter(StringView view, RxFragmentActivity activity) {
        super(view, activity);
    }
    ...
    public void delMsg() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getIntentionMsgDelApi().delMsg(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
