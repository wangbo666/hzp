...
public class ChildAccountListPresenter extends IBasePresenter<UserListView,RxActivity> {
    private final String TAG = ChildAccountListPresenter.class.getSimpleName();

    private UserListBean bean;

    public ChildAccountListPresenter(UserListView view, RxActivity activity) {
        super(view, activity);
    }
    ...
    public void getChildList() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getChildAccountListApi().getChildList(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
    }
}
