...
public class ChildAccountDelPresenter extends IBasePresenter<StringView,RxActivity> {
    private String msg="";

    private final String TAG = ChildAccountDelPresenter.class.getSimpleName();

    public ChildAccountDelPresenter(StringView view, RxActivity activity) {
        super(view, activity);
    }
    ...
    public void delChild() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getChildAccountDelApi().delChild(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
