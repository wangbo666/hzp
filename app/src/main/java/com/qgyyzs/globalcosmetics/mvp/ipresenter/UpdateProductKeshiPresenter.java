...
public class UpdateProductKeshiPresenter extends IBasePresenter<StringView,RxActivity> {
    private String msg="";

    private final String TAG = UpdateProductKeshiPresenter.class.getSimpleName();

    public UpdateProductKeshiPresenter(StringView view, RxActivity activity) {
        super(view, activity);
    }
    ...
    public void updateKeshi() {
        ...
        HttpRxObservable.getObservable(ApiUtils.getProductKeshiApi().getkeshi(AesUtils.AesString(getView().getJsonString())), getActivity()).subscribe(httpRxObserver);
