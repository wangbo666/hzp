...
public class AesUtils {
    ...
    private static String ivParameter = "7620795343827568";// 密钥默认偏移，可更改
    // private byte[] iv = { 0xA, 1, 0xB, 5, 4, 0xF, 7, 9, 0x17, 3, 1, 6, 8,
    // 0xC,
    // 0xD, 91 };
    private byte[] iv = ivParameter.getBytes();
    private IvParameterSpec IV;
    String sKey = "L#l48$s#0zlAw92G";// key必须为16位，可更改为自己的key
    ...
    public static String AesString(String jsonString){
        byte[] mBytes = null;
        AesUtils mAes = new AesUtils();
        try {
            mBytes = (TextUtils.isEmpty(jsonString)?"":jsonString).getBytes("UTF8");
        } catch (Exception e) {
        }
